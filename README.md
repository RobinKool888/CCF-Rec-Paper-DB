...
- [目录](#目录)
- [项目介绍](#项目介绍)
  - [CCF推荐会议与期刊（2026年第七版）](#CCF推荐会议与期刊（2026年第七版）)
  - [项目介绍](#项目介绍)
...
- 共计681个期刊/会议，其中7个期刊/会议有单独`url`，其余均可在`dblp`数据库检索。

  ```python
  no_dblp = ['HCC', 'HotSec', 'JASA', 'JSLHR', 'Cognition', 'IET Intelligent Transport Systems', 'EITEE']
  ```
...
- 由于681个期刊/会议中大多数均可在`dblp`数据库检索，`./src/parse_html.py`目前仅提供了`dblp`的解析，对其余7个期刊/会议，暂需自行到对应网站查找，后续有时间会补充对`no_dblp`期刊/论文的解析；
- 数据库的更新（*TODO*）
